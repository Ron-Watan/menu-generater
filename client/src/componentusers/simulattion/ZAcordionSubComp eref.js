...
const AcordionSubComp = (prop) => {
  ...
  let r = document.querySelectorAll('.texddddd')
  // r.forEach(el => {
  ...
  useEffect(() => {
    let r = document.querySelectorAll('.texddddd')
    let test = []
    // console.dir(r[0].scrollHeight)
    // console.dir(elRef.current?.scrollHeight)
    // if (!elRef?.current?.scrollHeight) return
    r.forEach(el => {
      test.push(el.scrollHeight)
    })
    setHema(test)
    // setHeight([...height,r[0].scrollHeight])
            ...
            <Accordion expanded={(expanded === el.panelCode || prop.accordian === false) && true} onChange={handleChange(el.panelCode)}>
              ...
              {prop.description && <AccordionDetails>
                ...
                {true &&
                  ...
                  <textarea readOnly
                    style={{
                      'height': `${hema[index]}px`,
                    }}

                    // ref={(element) => { ref.current[0] = element }}
                    ref={elRef}
                    // ref={(element) => { elRef.current[index] = element }}
                    value={prop.language === 1 ? el.remark : el.remark_2} name='remark' id='remark'
                    className='texddddd' />
